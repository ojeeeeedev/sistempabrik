...
export default function Landing() {
  ...
  useEffect(() => {
    const checkStatus = async () => {
      ...
      let newColor = 'bg-slate-500';
      let newText = 'Checking Status...';
      ...
      try {
        ...
        if (apiRes.ok) {
           ...
           if (failures === 0) {
             newColor = 'bg-emerald-700';
             newText = 'All Systems Operational';
           } else if (failures === values.length) {
             newColor = 'bg-red-700';
             newText = 'All Services Down';
           } else {
             newColor = 'bg-amber-600';
             newText = 'Partial Service';
           }
        } else {
  ...
  return (
    <div className="h-screen h-[100dvh] bg-[#0a2423] text-slate-200 flex flex-col overflow-hidden relative selection:bg-[#113b39]/50">
      ...
      {/* Footer */}
      <footer className="w-full bg-[#081c1b] border-t border-white/5 pt-3 pb-10 md:py-6 z-10 relative shrink-0">
        <div className="container mx-auto px-6 md:px-12">
          ...
          >
            ...
            {/* Copyright */}
            <div className="col-span-2 md:col-span-1 flex flex-col md:gap-3 md:text-right border-t border-white/5 pt-4 md:border-none md:pt-0 items-end">
              <div className="flex items-center justify-between md:flex-col md:items-end w-full gap-2">
                <p className="text-left md:text-right text-[8px] md:text-sm">&copy; {new Date().getFullYear()} PT. UTAMA KORINDAH.<br className="hidden md:block" /> All rights reserved.</p>
                
                {/* Status Button */}
                <Link to="/status" className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-white/10 transition-all text-[9px] md:text-xs font-semibold text-white group shadow-lg ${statusColor} hover:brightness-110 shrink-0`}>
                  {statusText.includes('Checking') ? (
                    <svg className="animate-spin h-2 w-2 md:h-3 md:w-3 text-white/80" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                  ) : (
                    <span className="w-1 h-1 md:w-2 md:h-2 rounded-full bg-white shadow-[0_0_8px_rgba(255,255,255,0.5)] group-hover:scale-125 transition-all"></span>
                  )}
                  <span className="md:hidden">
                    {statusText.replace('All Systems ', '').replace('All Services ', '').replace('All ', '').replace('Checking Status...', 'Checking...').replace(' Service', '')}
                  </span>
